...
func NewGitRepo(workdirRoot, remote string, localOK bool, offset string) (Repo, error) {
	r := &gitRepo{remote: remote, offset: offset}
	if strings.Contains(remote, "://") {
		// This is a remote path.
		var err error
		r.dir, err = WorkDir(workdirRoot, gitWorkDirType, r.remote)
		if err != nil {
			return nil, err
		}
...
func (r *gitRepo) loadRefs() {
	...
	if err != nil {
		...
		fmt.Println("Failed gettings regs", err)
		r.refsErr = err
		return
	}
...
func (r *gitRepo) ReadFile(rev, file string, maxSize int64) ([]byte, error) {
	...
	file = filepath.Join(r.offset, file)
...
func (r *gitRepo) ReadZip(rev, subdir string, maxSize int64) (zip io.ReadCloser, actualSubdir string, err error) {
	...
	args := []string{}
	subdir = filepath.Join(r.offset, subdir)
	if subdir != "" {
